...
import com.ncv.razorpay.merchant.mapper.WebhookConfigMapperImpl;
...
public class WebhookDeliveryExecutor {
    ...
    public void deliver(UUID webhookEventId) {
        ...
        try {
            ...
            log.error("Get RestClientException", e);
        }
    }
    private void handleAttemptFailed(WebhookEvent event,String error){
        ...
        log.error("Handling attempt failed for webhook event {} with attempts {}, Next retryAt {}",event.getId(),
                event.getAttempts(),event.getNextRetryAt());
    }
}
